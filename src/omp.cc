...
#pragma omp parallel for reduction(+:n_of_drop, total_stress_all, total_gpa_all, total_stress_drop, total_gpa_drop, total_stress_non, total_gpa_non, non_drop_students, count_m, count_f)
	...
	#pragma omp parallel for reduction(+:high_risk_pairs) schedule(dynamic)
	...
	//start = sc::now();
	#pragma omp parallel for reduction(+:trio_count) schedule(dynamic)
